...
public final class ConcurrentTimeInterpolatableBuffer<T> {
    ...
    public static <T> ConcurrentTimeInterpolatableBuffer<T> createBuffer(Interpolator<T> interpolateFunction, double historySizeSeconds) {
    ...
    public static <T extends Interpolatable<T>> ConcurrentTimeInterpolatableBuffer<T> createBuffer(double historySizeSeconds) {
    ...
    public Optional<T> getSample(double timeSeconds) {
        ...
        if (topBound == null && bottomBound == null) {
            ...
            // Otherwise, interpolate. Because T is between [0, 1], we want the ratio of (the difference between the current time and bottom bound) and (the difference between top and bottom bounds).
            return Optional.of(m_interpolatingFunc.interpolate(bottomBound.getValue(), topBound.getValue(), (timeSeconds - bottomBound.getKey()) / (topBound.getKey() - bottomBound.getKey())));
        }
    }
