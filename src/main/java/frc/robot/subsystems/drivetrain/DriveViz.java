...
public class DriveViz {
    ...
    private final MechanismLigament2d[] moduleDirections = new MechanismLigament2d[] {
        moduleMechanisms[0].getRoot("RootDirection", 0.5, 0.5).append(
            new MechanismLigament2d("Direction", 0.1, 0, 0, new Color8Bit(Color.kWhite))
        ),
        moduleMechanisms[1].getRoot("RootDirection", 0.5, 0.5).append(
            new MechanismLigament2d("Direction", 0.1, 0, 0, new Color8Bit(Color.kWhite))
        ),
        moduleMechanisms[2].getRoot("RootDirection", 0.5, 0.5).append(
            new MechanismLigament2d("Direction", 0.1, 0, 0, new Color8Bit(Color.kWhite))
        ),
        moduleMechanisms[3].getRoot("RootDirection", 0.5, 0.5).append(
            new MechanismLigament2d("Direction", 0.1, 0, 0, new Color8Bit(Color.kWhite))
        )
    };
    ...
    public void telemeterize(SwerveDriveState state) {
        ...
        Logger.recordOutput("Drivetrain/Viz/Speed", velocities.getNorm());
        Logger.recordOutput("Drivetrain/Viz/VelocityX", velocities.getX());
        Logger.recordOutput("Drivetrain/Viz/VelocityY", velocities.getY());
        Logger.recordOutput("Drivetrain/Viz/OdometryPerioid", state.OdometryPeriod);
        Logger.recordOutput("Drivetrain/Viz/MeasuredModuleStates", state.ModuleStates);
